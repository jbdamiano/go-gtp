...
Prepare functions that comform to [`HandlerFunc`](https://pkg.go.dev/github.com/jbdamiano/go-gtp/gtpv2#Conn.AddHandler), and register them to `Conn` with `AddHandler`. This should be done as soon as you get `Conn` not to miss the incoming messages.
