...
WORKDIR /opt
ENV TZ=Europe/Paris
RUN ln -snf /usr/share/zoneinfo/$TZ /etc/localtime && echo $TZ > /etc/timezone && apt update && apt upgrade -y && apt install -y git golang-go iproute2 tcpdump net-tools && git clone  https://github.com/jbdamiano/go-gtp.git -b dev2 /go-gtp 
