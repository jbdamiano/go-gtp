...
type sgw struct {
	...
	s11Conn, s5cConn *gtpv1.CPlaneConn
...
func newSGW(cfg *Config) (*sgw, error) {
	...
	var err error
	s.s11Addr, err = net.ResolveUDPAddr("udp", cfg.LocalAddrs.S11IP+gtpv1.GTPCPort)
	if err != nil {
		return nil, err
	}
	...
	s.s5cAddr, err = net.ResolveUDPAddr("udp", cfg.LocalAddrs.S5CIP+gtpv1.GTPCPort)
	if err != nil {
		return nil, err
	}
	...
	s.s1uAddr, err = net.ResolveUDPAddr("udp", cfg.LocalAddrs.S1UIP+gtpv1.GTPUPort)
	if err != nil {
		return nil, err
	}
	...
	s.s5uAddr, err = net.ResolveUDPAddr("udp", cfg.LocalAddrs.S5UIP+gtpv1.GTPUPort)
	if err != nil {
		return nil, err
	}
...
func (s *sgw) run(ctx context.Context) error {
	s.s11Conn = gtpv1.NewConn(s.s11Addr, 5, 0, 4)
	go func() {
	...
	s.s5cConn = gtpv1.NewConn(s.s5cAddr, 6, 0, 4)
	go func() {
	...
	s.s11Conn.AddHandlers(map[uint8]gtpv1.HandlerFunc{
		message.MsgTypeCreatePDPContextRequest: s.handleCreateSessionRequest,
		message.MsgTypeUpdatePDPContextRequest: s.handleModifyBearerRequest,
		message.MsgTypeDeletePDPContextRequest: s.handleDeleteSessionRequest,
	})
