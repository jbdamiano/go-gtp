...
func NewConn(laddr net.Addr, localIfType, counter uint8, node uint8) *CPlaneConn {
	return &CPlaneConn{
		mu:                sync.Mutex{},
		laddr:             laddr,
...
func Dial(ctx context.Context, laddr, raddr net.Addr, localIfType, counter uint8) (*CPlaneConn, error) {
	c := &CPlaneConn{
		mu:                sync.Mutex{},
		laddr:             laddr,
		imsiSessionMap:    newimsiSessionMap(),
...
func (c *CPlaneConn) ListenAndServe(ctx context.Context) error {
	err := c.Listen(ctx)
	if err != nil {
		return err
	}
...
func (c *CPlaneConn) Listen(ctx context.Context) error {
	var err error
	c.mu.Lock()
	c.pktConn, err = net.ListenPacket(c.laddr.Network(), c.laddr.String())
	c.mu.Unlock()
...
func (c *CPlaneConn) closed() <-chan struct{} {
	return c.closeCh
}

// Serve starts serving GTPv2 connection.
func (c *CPlaneConn) Serve(ctx context.Context) error {
	go func() {
...
func (c *CPlaneConn) ReadFrom(p []byte) (n int, addr net.Addr, err error) {
...
func (c *CPlaneConn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	return c.pktConn.WriteTo(p, addr)
}

// Close closes the connection.
// Any blocked Read or Write operations will be unblocked and return errors.
func (c *CPlaneConn) Close() error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CPlaneConn) LocalAddr() net.Addr {
...
func (c *CPlaneConn) SetDeadline(t time.Time) error {
...
func (c *CPlaneConn) SetReadDeadline(t time.Time) error {
...
func (c *CPlaneConn) SetWriteDeadline(t time.Time) error {
...
func (c *CPlaneConn) AddHandler(msgType uint8, fn HandlerFunc) {
...
func (c *CPlaneConn) AddHandlers(funcs map[uint8]HandlerFunc) {
...
func (c *CPlaneConn) Restarts() uint8 {
	return c.RestartCounter
}

func (c *CPlaneConn) handleMessage(senderAddr net.Addr, msg message.Message) error {
...
func (c *CPlaneConn) EnableValidation() {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CPlaneConn) DisableValidation() {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.validationEnabled = false
}

func (c *CPlaneConn) validate(senderAddr net.Addr, msg message.Message) error {
	// check GTP version
	if msg.Version() != 1 {
...
func (c *CPlaneConn) SendMessageTo(msg message.Message, addr net.Addr) (uint16, error) {
	seq := c.IncSequence()
...
func (c *CPlaneConn) IncSequence() uint16 {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.sequence++

	// SequenceNumber is 2-octet long
	if c.sequence > 0xffff {
		c.sequence = 0
	}
...
func (c *CPlaneConn) DecSequence() uint16 {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.sequence--
...
func (c *CPlaneConn) SequenceNumber() uint16 {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *CPlaneConn) EchoRequest(raddr net.Addr) (uint16, error) {
...
func (c *CPlaneConn) EchoResponse(raddr net.Addr, req message.Message) error {
...
func (c *CPlaneConn) ParseCreateSession(raddr net.Addr, ies ...*ie.IE) (*Session, error) {
	...
	for _, i := range ies {
		...
		case ie.IMEISV:
			sess.IMEI, err = i.IMEISV()
			if err != nil {
				return nil, err
			}
		case ie.CellIdentification:
			sess.MCC, err = i.MCC()
			if err != nil {
				return nil, err
			}
...
// this method.
func (c *CPlaneConn) CreateSession(raddr net.Addr, ie ...*ie.IE) (*Session, uint16, error) {
	...
	msg := message.NewCreatePDPContextRequest(0, 0, ie...)
...
func (c *CPlaneConn) ModifyBearer(teid uint32, sess *Session, ie ...*ie.IE) (uint16, error) {
	msg := message.NewUpdatePDPContextRequest(teid, 0, ie...)
...
func (c *CPlaneConn) DeleteSession(teid uint32, sess *Session, ie ...*ie.IE) (uint16, error) {
	msg := message.NewDeletePDPContextRequest(teid, 0, ie...)
...
/*
// ModifyBearer sends a ModifyBearerRequest with TEID and IEs given..
func (c *Conn) ModifyBearer(teid uint32, sess *Session, ie ...*ie.IE) (uint16, error) {
...
func (c *Conn) DeleteBearer(teid uint32, sess *Session, ie ...*ie.IE) (uint16, error) {
...
func (c *CPlaneConn) RespondTo(raddr net.Addr, received, toBeSent message.Message) error {
...
func (c *CPlaneConn) GetSessionByTEID(teid uint32, peer net.Addr) (*Session, error) {
...
func (c *CPlaneConn) GetSessionByIMSI(imsi string) (*Session, error) {
...
func (c *CPlaneConn) GetIMSIByTEID(teid uint32, peer net.Addr) (string, error) {
...
func (c *CPlaneConn) RegisterSession(itei uint32, session *Session) {
...
func (c *CPlaneConn) RemoveSession(session *Session) {
...
func (c *CPlaneConn) RemoveSessionByIMSI(imsi string) {
...
func (c *CPlaneConn) NewSenderCTEID() (fteidIE *ie.IE) {
	var teid uint32
...
func (c *CPlaneConn) NewSenderUTEID() (fteidIE *ie.IE) {
	var teid uint32
	for try := uint32(0); try < 0xffff; try++ {
		const logEvery = 0xff
		if try&logEvery == logEvery {
...
func (c *CPlaneConn) Sessions() []*Session {
	var ss []*Session
...
func (c *CPlaneConn) SessionCount() int {
	var count int
	c.imsiSessionMap.rangeWithFunc(func(k, v interface{}) bool {
		sess := v.(*Session)
		if sess.IsActive() {
